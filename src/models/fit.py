...
def lstm_rnn(enc,dec,train,valid,epochs,opt_enc,opt_dec,loss_fn, trg_vocab_size, trg_sos_id,
             src_name, trg_name, ds_name, model_name, teacher_forcing=0.5, end_train=300,
             end_val=100, print_every=100):
    ...
    def getLoss(val=False):
        ...
        example = 1
        for pair in data:
            src,trg = getattr(pair,src_name),getattr(pair,trg_name)
            hidd = enc(src)
    ...
    for e in range(1,epochs+1):
        ...
        for pair in train:
            ...
            if example % print_every == 0:
                ...
                avg_loss = 0
                torch.save(enc.state_dict(), f'models/{model_name}/{ds_name}/enc_{checkpoint}.pt')
                torch.save(dec.state_dict(), f'models/{model_name}/{ds_name}/dec_{checkpoint}.pt')
                checkpoint += 1
...
def lstm_rnn_vls(enc,dec,train,valid,epochs,opt_enc,opt_dec,loss_fn, trg_vocab_size, trg_sos_id, pad_de, pad_en,
...
def rnnsearch(enc, dec, train, valid, epochs, opt_enc, opt_dec, loss_fn, trg_vocab_size,
    ...
    for e in range(1, epochs + 1):
        ...
        for _ in tqdm(range(end_train)):
            ...
            if example % batch_size == 0 or example == end_train:
                opt_enc.step()
                opt_dec.step()
            ...
            if example % save_every == 0:
                torch.save(enc.state_dict(), f'models/RNNsearch/Multi30k/enc_{checkpoint}.pt')
                torch.save(dec.state_dict(), f'models/RNNsearch/Multi30k/dec_{checkpoint}.pt')
                checkpoint += 1
